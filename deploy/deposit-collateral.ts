...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // await (await token.approve(crossMarginHandler.address, ethers.constants.MaxUint256)).wait();

  await (
    await crossMarginHandler.depositCollateral(deployer.address, 0, token.address, ethers.utils.parseUnits("1", 6), {
      gasLimit: 10000000,
    })
  ).wait();
};
