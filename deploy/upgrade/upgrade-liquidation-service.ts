...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const TARGET_ADDRESS = config.services.liquidation;

  console.log(`> Preparing to upgrade LiquidationService`);
  const newImplementation = await upgrades.prepareUpgrade(TARGET_ADDRESS, Contract);
  console.log(`> Done`);

  console.log(`> New LiquidationService Implementation address: ${newImplementation}`);
...
export default func;
func.tags = ["UpgradeLiquidationService"];
