...
abstract contract BaseTest is TestBase, Deployment, StdAssertions {
  ...
  // --------- Setup Helpers ---------
  function setupDefaultUnderlying() internal view returns (address[] memory, PoolConfig.UnderlyingConfig[] memory) {
    ...
    PoolConfig.UnderlyingConfig[] memory underlyingConfigs = new PoolConfig.UnderlyingConfig[](4);
    underlyingConfigs[0] = PoolConfig.UnderlyingConfig({ isAccept: true, decimals: weth.decimals(), weight: 100 });
    underlyingConfigs[1] = PoolConfig.UnderlyingConfig({ isAccept: true, decimals: wbtc.decimals(), weight: 100 });
    underlyingConfigs[2] = PoolConfig.UnderlyingConfig({ isAccept: true, decimals: dai.decimals(), weight: 100 });
    underlyingConfigs[3] = PoolConfig.UnderlyingConfig({ isAccept: true, decimals: usdc.decimals(), weight: 100 });
