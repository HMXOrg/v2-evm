...
const marketConfig = {
  ...
  fundingRate: {
    ...
    maxFundingRate: ethers.utils.parseUnits("0.1", 18), // 10% per day
  },
...
async function main() {
  ...
  console.log("> ConfigStorage: Set Market Config...");
  await (await configStorage.setMarketConfig(marketIndex, marketConfig)).wait();
  console.log("> ConfigStorage: Set Market Config success!");
}
main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
