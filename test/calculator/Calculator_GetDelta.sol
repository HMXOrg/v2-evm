...
contract Calculator_GetDelta is Calculator_Base {
  ...
  function testRevert_getDelta_WhenBadAveragePrice() external {
    ...
    (, uint256 delta) = calculator.getDelta(size, isLong, 1e30, avgPriceE30, 0, 0);
  ...
  function testCorrectness_getDelta_WhenLongAndPriceUp() external {
    ...
    (bool isProfit, uint256 delta) = calculator.getDelta(size, isLong, nextPrice, avgPriceE30, 0, 0);
  ...
  function testCorrectness_getDelta_WhenProfit_ButHaventSurpassMinProfitDuration() external {
    ...
    // price up -> profit
    (bool isProfit, uint256 delta) = calculator.getDelta(size, isLong, nextPrice, avgPriceE30, lastIncrease, 0);
    assertEq(isProfit, true);
    assertEq(delta, 0);
    ...
    // price up -> profit
    (isProfit, delta) = calculator.getDelta(size, isLong, nextPrice, avgPriceE30, lastIncrease, 0);
    assertEq(isProfit, true);
