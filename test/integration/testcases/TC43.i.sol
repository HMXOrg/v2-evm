...
contract TC43 is BaseIntTest_WithActions {
  ...
  function testCorrectness_TC43_intentHandler_payExecutionFeeWithDai() external {
    uint256 privateKey = uint256(keccak256(bytes("1")));
    BOB = vm.addr(privateKey);
    ...
    dai.mint(BOB, 100_000 * 1e18);
    depositCollateral(BOB, 0, ERC20(address(dai)), 100_000 * 1e18);
    {
      assertVaultTokenBalance(address(dai), 100_000 * 1e18, "TC43: after deposit collateral");
    }
    ...
    // 1st trade's trading fee = 100000 - 100 = 99900
    // 1st trade's execution fee = 99900 - 0.1 = 99899.9
    assertEq(vaultStorage.traderBalances(BOB, address(dai)), 99899.9 * 1e18);
  }
}
