...
library Deployer {
  ...
  function deployCIXPriceAdapter(address _proxyAdmin) internal returns (ICIXPriceAdapter) {
    bytes memory _logicBytecode = abi.encodePacked(vm.getCode("./out/CIXPriceAdapter.sol/CIXPriceAdapter.json"));
    bytes memory _initializer = abi.encodeWithSelector(bytes4(keccak256("initialize()")));
    address _proxy = _setupUpgradeable(_logicBytecode, _initializer, _proxyAdmin);
    return ICIXPriceAdapter(payable(_proxy));
  }
