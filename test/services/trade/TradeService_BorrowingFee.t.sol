...
contract TradeService_BorrowingFee is TradeService_Base {
  ...
  function testCorrectness_borrowingFee_WhenIncreasePosition() external {
    ...
    vm.warp(100);
    {
      ...
      assertEq(vaultStorage.plpLiquidity(address(usdt)), 0);
    ...
    vm.warp(101);
    {
      ...
      // 0.81 * 85% = 0.6885
      assertEq(vaultStorage.plpLiquidity(address(usdt)), 0.6885 * 1e6);
      // 0.81 * 15% = 0.1215
    ...
    vm.warp(120);
    {
      ...
      assertEq(vaultStorage.plpLiquidity(address(usdt)), 8.5 * 1e6);
  ...
  function testCorrectness_borrowingFee_WhenDecreasePosition() external {
    ...
    vm.warp(120);
    {
      ...
      assertEq(vaultStorage.plpLiquidity(address(usdt)), 12.75 * 1e6);
  ...
  function testCorrectness_borrowingFee() external {
    ...
    vm.warp(150);
    {
      ...
      assertEq(vaultStorage.plpLiquidity(address(weth)), 0.0085 * 1e18);
      // 0.002025 * 85% = 0.00172125
      assertEq(vaultStorage.plpLiquidity(address(wbtc)), 0.00172125 * 1e8);
      // 100 * 85% = 85
      assertEq(vaultStorage.plpLiquidity(address(usdt)), 85 * 1e6);
